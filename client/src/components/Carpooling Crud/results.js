...
export class Results extends Component {
    ...
    componentDidMount() {
        console.log(this.props.location.data.from.value)

        const { from, to } = this.state; //object disctructor
        const carpooling = {

            trage: {
                from: this.props.location.data.from.value,
                to: this.props.location.data.to.value
            },
            people_parcel_Carpooling: this.props.location.data.people_parcel_Carpooling.value,
            offre_demand_Carpooling:this.props.location.data.offre_demand_Carpooling.value,
        ...
        axios.post(`carpooling/searchByType`, carpooling)
            .then(async res => {
                ...
                if (res.status === 200) {
                    this.setState({ carpoolings: res.data.data })
